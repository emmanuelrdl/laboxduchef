class MealsController < ApplicationController
  ...
  def index
    @meal = Meal.new
    @meals = Meal.all.paginate(:page => params[:page], :per => 6)


    where_group = params[:full_addressuser_input_autocomplete_address]
    if where_group
    @meals = Meal.joins(:restaurant).near(params[:full_addressuser_input_autocomplete_address], 20, order: 'distance').paginate(:page => params[:page], :per => 6)
    end
